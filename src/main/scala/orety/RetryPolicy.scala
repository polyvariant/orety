...
case class RetryPolicy[F[_], -Res](
  ...
  def followedBy[R <: Res](rp: RetryPolicy[F, R])(using F: Apply[F]): RetryPolicy[F, R] = RetryPolicy.withShow[F, R](
    (actionResult, status) =>
  ...
  /** Combine this schedule with another schedule, giving up when either of the schedules want to give up and
    ...
    */
  def join[R <: Res](rp: RetryPolicy[F, R])(using F: Apply[F]): RetryPolicy[F, R] = RetryPolicy.withShow[F, R](
    (actionResult, status) =>
      F.map2(decideNextRetry(actionResult, status), rp.decideNextRetry(actionResult, status)) {
  ...
  /** Combine this schedule with another schedule, giving up when both of the schedules want to give up and
    ...
    */
  def meet[R <: Res](rp: RetryPolicy[F, R])(using F: Apply[F]): RetryPolicy[F, R] = RetryPolicy.withShow[F, R](
    (actionResult, status) =>
      F.map2(decideNextRetry(actionResult, status), rp.decideNextRetry(actionResult, status)) {
  ...
  )(using F: Functor[F]): RetryPolicy[F, Res] = RetryPolicy.withShow(
    (actionResult, status) =>
  ...
  )(using F: Monad[F]): RetryPolicy[F, Res] = RetryPolicy.withShow(
    (actionResult, status) =>
  ...
  def mapK[N[_]](nt: FunctionK[F, N]): RetryPolicy[N, Res] = RetryPolicy.withShow(
    (actionResult, status) => nt(decideNextRetry(actionResult, status)),
    show"$show.mapK(<FunctionK>)"
  )

  def contramap[R](f: R => Res): RetryPolicy[F, R] = RetryPolicy.withShow[F, R](
...
object RetryPolicy:
  ...
  def withShow[F[_], Res](
      decideNextRetry: (Res, RetryStatus) => F[PolicyDecision],
      pretty: => String
  ): RetryPolicy[F, Res] = new RetryPolicy[F, Res](decideNextRetry):
  ...
  given [F[_], Res](using
      F: Applicative[F]
  ): BoundedSemilattice[RetryPolicy[F, Res]] = new BoundedSemilattice[RetryPolicy[F, Res]]:
    override def empty: RetryPolicy[F, Res] = RetryPolicies.constantDelay[F](Duration.Zero)
  ...
  given [F[_], Res]: Show[RetryPolicy[F, Res]] = Show.show(_.show)
end RetryPolicy
